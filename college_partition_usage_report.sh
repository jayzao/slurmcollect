...
FILENAME=college_partition_usage
...
echo -e "COLLEGE_NAME,COLLEGE_DESCRIPTION,COLLEGE_REPORTNAME,PARTITION,CPU_HOURS" >> /tmp/template
...
group by
	college,show_completed_jobs.partition
order by
	college"
...
echo -e "${A2C2_CLUSTERNAME} college partition usage report between $START and $END\n\nGenerated at $(date +'%F @ %T') for $EMAIL" | \
    mutt -F $MUTTCONFIG \
